...
""""
Updated with active uplink specification
Original version could not customize active uplinks but only inherited DVS properties
"""
...
options:
    ...
    act_links:
        description:
            - List which configures the active links in teaming settings for the portgroup.
        required: False
...
EXAMPLES = '''
   ...
   - name: Create vlan portgroup with specific uplinks
     connection: local
     vmware_dvs_portgroup:
        hostname: vcenter_ip_or_hostname
        username: vcenter_username
...
class VMwareDvsPortgroup(PyVmomi):
    def __init__(self, module):
        ...
        self.active_links = self.module.params['act_links']
    ...
    def create_port_group(self):
        ...
        config.defaultPortConfig.uplinkTeamingPolicy.uplinkPortOrder = vim.dvs.VmwareDistributedVirtualSwitch.UplinkPortOrderPolicy()
        if self.active_links:
            config.defaultPortConfig.uplinkTeamingPolicy.uplinkPortOrder.inherited = False
            config.defaultPortConfig.uplinkTeamingPolicy.uplinkPortOrder.activeUplinkPort = []
            config.defaultPortConfig.uplinkTeamingPolicy.uplinkPortOrder.activeUplinkPort = self.active_links
        else:
            config.defaultPortConfig.uplinkTeamingPolicy.uplinkPortOrder.inherited = True
